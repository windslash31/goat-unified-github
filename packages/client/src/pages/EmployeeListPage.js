...
export const EmployeeListPage = ({ employees, isLoading, filters, setFilters, pagination, setPagination, sorting, setSorting, onEdit, onDeactivate }) => {
    ...
    return (
        ...
        >
                ...
                {isDesktop && selectedRows.size > 0 ? (
                    <div className="w-full flex justify-between items-center bg-kredivo-light p-2 rounded-lg">
                        <div className="flex items-center gap-2">
                             <button onClick={() => setSelectedRows(new Set())} className="p-2 text-kredivo-dark-text rounded-full hover:bg-kredivo-primary/20">
                                <X className="w-5 h-5" />
                            </button>
                            <span className="font-semibold text-kredivo-dark-text">{selectedRows.size} selected</span>
                        </div>
                        <div className="relative">
                            <Button onClick={() => setIsBulkActionMenuOpen(prev => !prev)} variant="primary">
                                Actions
                            </Button>
            ...
            <FilterPills filters={filters} setFilters={setFilters} setSearchInputValue={setSearchInputValue} options={filterOptions} onClear={handleClearFilters} />
