...
const FilterPills = ({ filters, setFilters, setSearchInputValue }) => {
    ...
    const activeFilters = Object.entries(filters)
        ...
        .map(([key, value]) => ({
            key,
            label: key === 'jobTitle' ? 'Job Title' : capitalize(key),
...
export const EmployeeListPage = ({ employees, onSelectEmployee, filters, setFilters, pagination, setPagination, sorting, setSorting }) => {
    ...
    }, [debouncedSearchTerm, setFilters, setPagination]);
    ...
    const handleClearFilters = () => {
        const cleared = { status: 'all', search: '', jobTitle: '', manager: '' };
        setFilters(cleared);
        setPagination(prev => ({ ...prev, currentPage: 1 }));
        setSearchInputValue('');
    };
    
    // MODIFIED: An advanced filter is now any filter that is not the default
    const areAdvancedFiltersActive = useMemo(() => {
        return filters.status !== 'all' || filters.jobTitle || filters.manager;
    }, [filters]);
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-2">
                ...
                <div className="flex items-center gap-4 w-full sm:w-auto">
                    ...
                    <div className="relative">
                        ...
                        <div ref={popoverRef}>
                            {/* MODIFIED: The popover is now used for all filters */}
                            {isFilterPopoverOpen && <FilterPopover initialFilters={filters} onApply={setFilters} onClear={handleClearFilters} onClose={() => setIsFilterPopoverOpen(false)} />}
                        </div>
                    </div>
                </div>
            </div>

            <FilterPills filters={filters} setFilters={setFilters} setSearchInputValue={setSearchInputValue} />
            
            <div className="mt-2 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                        ...
                        {employees.length > 0 ? employees.map(emp => {
                            ...
                            return (
                                <tr key={emp.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium"><button onClick={() => onSelectEmployee(emp.id)} className="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300">View Profile</button></td>
                                </tr>
                            );
                        }) : (
