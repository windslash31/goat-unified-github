    ...
    <div>
      ...
      <p className="text-sm text-gray-500">
        ...
        <span className="font-medium">
          {details.expires
            ? new Date(details.expires).toLocaleString()
            : "Never"}
        </span>
      </p>
    </div>
...
const PermissionChangeDetail = ({ changes }) => {
  ...
  if (added.length === 0 && removed.length === 0) {
    return (
      <span className="text-gray-500">No permission changes detected.</span>
    );
  }
  return (
...
export const ActivityLogPage = ({ onLogout }) => {
  ...
  const handleExport = async () => {
    try {
      const response = await api.get("/api/logs/activity/export", {
        responseType: "blob",
      });
  ...
  const renderLogDetails = (log) => {
    ...
    switch (log.action_type) {
      case "APPLICATION_ACCESS_CREATE":
        return <ApplicationAccessDetail details={log.details.details} />;
      case "EMPLOYEE_CREATE":
        return <EmployeeCreateDetail details={log.details} />;
      case "USER_PASSWORD_CHANGE_SUCCESS":
        return <PasswordChangeSuccessDetail />;
      case "ADMIN_PASSWORD_RESET":
        return <AdminPasswordResetDetail details={log.details} />;
      case "API_KEY_CREATE":
        return <ApiKeyCreateDetail details={log.details} />;
      case "API_KEY_DELETE":
        return <ApiKeyDeleteDetail />;
      case "USER_LOGOUT_SUCCESS":
        return <LogoutSuccessDetail />;
      case "ROLE_CREATE":
        return <RoleCreateDetail details={log.details} />;
      case "ROLE_DELETE":
        return <RoleDeleteDetail details={log.details} />;
      case "USER_LOGIN_SUCCESS":
        return <UserLoginSuccessDetail details={log.details} />;
      ...
      default:
        return <GenericDetail details={log.details} />;
    }
  };
          ...
          logs.map((log) => (
            ...
            >
              <div className="flex justify-between items-start">
                <div
                  className={`font-semibold text-base ${getActionTypeStyles(
                    log.action_type
                  )}`}
                >
                  ...
                  >
                    <ChevronRight
                      className={`w-5 h-5 transition-transform ${
                        expandedLogRowId === log.id ? "rotate-90" : ""
                      }`}
                    />
                  </button>
                )}
              ...
              <div className="mt-2 text-sm text-gray-600 dark:text-gray-400 space-y-1">
                <p>
                  ...
                  {log.actor_email || "System (or Deleted User)"}
                </p>
                <p>
                  <strong className="font-medium text-gray-800 dark:text-gray-200">
                    Target:
                  </strong>{" "}
                  {getTargetDisplayName(log)}
                </p>
      ...
      <div className="mt-4 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden hidden md:block">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead className="bg-gray-50 dark:bg-gray-700/50">
            <tr>
              <th className="w-1/4 px-6 py-3 text-left text-xs font-bold uppercase">
                Timestamp
              </th>
              <th className="w-1/4 px-6 py-3 text-left text-xs font-bold uppercase">
                Actor
                ...
                <React.Fragment key={log.id}>
                  ...
                  {expandedLogRowId === log.id && (
                    <tr>
                      <td
                        colSpan="4"
                        className="p-4 bg-gray-100 dark:bg-gray-900/50"
                      >
                        <LogDetailComponent log={log} />
                      </td>
                    </tr>
