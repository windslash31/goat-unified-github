...
const CreateEmployeeModal = lazy(() =>
  import("./components/ui/CreateEmployeeModal").then((module) => ({
    default: module.CreateEmployeeModal,
  }))
);
const ImportEmployeesModal = lazy(() =>
  import("./components/ui/ImportEmployeesModal").then((module) => ({
    default: module.ImportEmployeesModal,
  }))
);
...
const AppContent = () => {
  const { authStatus, user, logout, verifyAuth } = useAuthStore();
  ...
  const {
    ...
    enabled:
      authStatus === "authenticated" && location.pathname === "/employees",
