...
const fetchTicketDetails = async (ticketId, token) => {
  ...
  const response = await fetch(
    `${process.env.REACT_APP_API_BASE_URL}/api/jira/ticket/${ticketId}`,
    {
...
const DetailRow = ({ icon, label, value }) => (
  <div className="flex items-center text-sm">
    ...
    <div className="w-2/3 text-gray-800 dark:text-gray-200 font-medium">
      {value}
    </div>
  </div>
);
...
export const JiraTicketModal = ({ ticketId, onClose }) => {
  ...
  const jiraBaseUrl =
    ...
    `https://${process.env.ATLASSIAN_DOMAIN}/browse/`;
