...
const PlatformIcon = ({ platform }) => {
    ...
    const colors = { google: 'bg-red-500', slack: 'bg-purple-500', jumpcloud: 'bg-orange-500', atlassian: 'bg-blue-500' };
    return (
        <div className={`w-8 h-8 rounded-md flex items-center justify-center text-white font-bold text-sm ${colors[platform] || 'bg-gray-500'}`}>
            {icons[platform] || '?'}
        </div>
    );
};

export const DeactivateEmployeeModal = ({ employee, onClose, onDeactivateSuccess }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (!response.ok) throw new Error(data.message || 'Failed to process suspensions.');
            
            const successCount = data.results.filter(r => r.status === 'SUCCESS').length;
            const failCount = data.results.filter(r => r.status === 'FAILED').length;
            toast.success(`Suspension process finished. Success: ${successCount}, Failed: ${failCount}.`);
            
            // The onDeactivateSuccess callback might refresh data, so we call it.
    ...
    return (
        // Added p-4 to the backdrop to prevent modal touching screen edges
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50">
            {/* Changed max-w-lg to a more responsive max-w-md */}
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md">
                <div className="p-6">
                    <div className="flex items-start">
                        <ShieldAlert className="h-8 w-8 text-red-500 mr-4 flex-shrink-0" />
                        <div>
                            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Manual Platform Suspension</h3>
                            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                Manually suspend <span className="font-bold">{employee.first_name} {employee.last_name}</span>'s access to external platforms. This is a failsafe and does not change their status in this application.
                            </p>
                        </div>
                    </div>
