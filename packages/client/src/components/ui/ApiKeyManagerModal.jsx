...
export const ApiKeyManagerModal = ({ user, onClose }) => {
    ...
    const handleGenerateKey = async (e) => {
        ...
        try {
            const { data } = await api.post(`/api/users/${user.id}/api-keys`, { description, expiresInDays });
                ...
                >
                    ...
                    >
                        <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                            <h3 className="text-lg font-semibold">API Keys for {user.full_name}</h3>
                        ...
                        <div className="p-6 flex-1 overflow-y-auto">
                            {loading ? <p>Loading keys...</p> : (
                                <ul className="space-y-3">
                                    {keys.map(key => (
                                        <li key={key.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                                            <div>
                                                <p className="font-semibold">{key.description}</p>
                                                <p className="text-xs text-gray-500">
                                                    Created: {new Date(key.created_at).toLocaleDateString()}
                                                    {key.expires_at ? ` | Expires: ${new Date(key.expires_at).toLocaleDateString()}` : ' | No Expiration'}
                                                </p>
                                            </div>
                                            <Button onClick={() => openDeleteModal(key)} variant="danger" className="px-2 py-1 text-xs">
                                                <Trash2 className="w-3 h-3 mr-1" /> Revoke
                                            </Button>
                                        </li>
                                    ))}
                                    {keys.length === 0 && <p className="text-center text-gray-500 py-4">No API keys found for this user.</p>}
                                </ul>
                            )}
                        </div>
                        
                        <form onSubmit={handleGenerateKey} className="p-6 border-t border-gray-200 dark:border-gray-700">
                            ...
                            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                                ...
                                <Button type="submit" disabled={isSubmitting} variant="primary" className="w-full sm:w-auto justify-center">
