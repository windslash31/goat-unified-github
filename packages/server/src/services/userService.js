...
const createUser = async (userData, actorId, reqContext) => {
  ...
  try {
    ...
    const employeeId =
      employeeResult.rows.length > 0 ? employeeResult.rows[0].id : null;
    ...
    await logActivity(
      ...
      "USER_CREATE",
      {
        ...
        createdUser: { fullName, email, roleId },
      },
      reqContext,
      client
    );
...
const updateUserRole = async (userId, newRoleId, actorId, reqContext) => {
  ...
  try {
    ...
    await logActivity(
      ...
      "USER_ROLE_UPDATE",
      {
        changes: { role: { from: oldRoleName, to: newRoleName } },
...
const deleteUser = async (userId, actorId, reqContext) => {
  ...
  try {
    await client.query("BEGIN");
    const userToDeleteResult = await client.query(
    ...
    await logActivity(
      ...
      "USER_DELETE",
      {
        targetUserEmail: deletedUserEmail,
        details: `Deleted user ${deletedUserEmail}`,
      },
      reqContext,
      client
    );
...
const changePassword = async (userId, oldPassword, newPassword, reqContext) => {
  ...
  if (!complexityCheck.valid)
    throw new Error(complexityCheck.issues.join(", "));
...
const resetPassword = async (targetUserId, actorId, reqContext) => {
  const targetUserResult = await db.query(
    "SELECT r.name as role_name, u.email FROM users u JOIN roles r ON u.role_id = r.id WHERE u.id = $1",
    [targetUserId]
  );
  ...
  if (!passwordValidation.valid) {
    // This is unlikely to happen with the random generator, but it's good practice
    // to handle it. A more robust solution would be to regenerate the password.
    throw new Error(
      "Generated temporary password does not meet complexity requirements."
    );
  }
  ...
  await logActivity(
    ...
    { targetUserEmail: targetUserResult.rows[0].email },
    reqContext
  );
...
const generateApiKey = async (
  ...
  await logActivity(
    actorId,
    "API_KEY_CREATE",
    { targetUserId: userId, keyId: result.rows[0].id, expires: expiresAt },
    reqContext
  );
