...
const getUserStatus = async (email) => {
    ...
    if (!process.env.ATLASSIAN_API_TOKEN || !process.env.ATLASSIAN_API_USER || !process.env.ATLASSIAN_DOMAIN) {
        console.error("Atlassian environment variables are not fully configured.");
        return { platform: 'Atlassian', email, status: 'Error', message: 'Server configuration error.' };
    }

    // Construct the URL using your ATLASSIAN_DOMAIN variable
    const url = `https://${process.env.ATLASSIAN_DOMAIN}/rest/api/3/user/search?query=${encodeURIComponent(email)}`;
...
const getTicketDetails = async (ticketId) => {
    if (!process.env.ATLASSIAN_DOMAIN || !process.env.ATLASSIAN_API_USER || !process.env.ATLASSIAN_API_TOKEN) {
        throw new Error('Atlassian API credentials are not configured on the server.');
    }
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Jira API responded with status ${response.status}: ${errorBody}`);
        }
...
const deactivateUser = async (email) => {
    ...
    return { success: true, message: `Deactivate action for ${email} would be performed here.` };
};
