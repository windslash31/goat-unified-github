...
const createUser = async (req, res, next) => {
  try {
    ...
    const newUser = await userService.createUser(req.body, actor, reqContext);
    res.status(201).json(newUser);
  } catch (error) {
    if (error.message.includes("already exists")) {
      return res.status(409).json({ message: error.message });
    }
    if (error.message.includes("permission to create an admin user")) {
...
const generateApiKey = async (req, res, next) => {
  try {
    ...
    if (!description) {
      return res
        .status(400)
        .json({ message: "A description for the API key is required." });
    }
